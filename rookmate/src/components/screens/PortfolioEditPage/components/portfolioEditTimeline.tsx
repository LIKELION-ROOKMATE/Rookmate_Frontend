...
const PortfolioViewTimeline: React.FC<PortfolioViewTimelineType> = ({
  ...
  const timelineAddEvent = (
    e: React.MouseEvent<HTMLButtonElement, MouseEvent>
  ) => {
    e.preventDefault();
    setTimelineElement((prev: any) => [
      ...prev,
      <div style={styles.timelineElement}>
        <div style={styles.point}></div>
        <div style={styles.titleContent}>
  ...
  return (
    <div>
      <div style={styles.imgTimelineBox}>
        <img src={mainImage} alt="main-img" style={styles.mainImg} />
        <div style={styles.editButtonBox}>
          <input
            type="file"
            style={{ display: "none" }}
            id="mainImage"
            onChange={addMainImageEvent}
          />
          <label style={styles.editButton} htmlFor="mainImage">
            대표이미지 설정
          </label>
          <button style={styles.editButton} onClick={timelineAddEvent}>
            타임라인 추가
          </button>
        </div>
