...
const AddWorkModal: React.FC<addWorkModalType> = ({
  setWorkImageList,
  setModalActive,
}) => {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < fieldList.length; i++) {
      setFieldElement((prev: any) => [
        ...prev,
        <div style={styles.fieldCheckBox}>
          <label htmlFor={fieldList[i]} style={styles.fieldName}>
            {fieldList[i]}
          </label>
          <input
            type="radio"
  ...
  return (
    <div style={styles.modalContainer}>
      <p style={{ fontSize: "0.75rem", fontWeight: "600", marginTop: "2rem" }}>
        작업 추가하기
      </p>
      ...
      <div style={styles.toolBox}>
        ...
        <div style={styles.tools}>
          ...
          <button style={styles.saveButton} onClick={addWorkImageEvent}>
            완료
          </button>
        </div>
      </div>
      <div style={styles.addWorkContent}>
        <div style={styles.workExplain}>
          <textarea
            placeholder="최대 500자"
            style={styles.workExplainContent}
          ...
          <div style={{ display: "flex", gap: "0.7rem" }}>
            ...
            <div style={styles.optionContent}>
              <div
                ...
                onClick={() => {
                  setViewField((prev) => !prev);
                }}
              >
                클릭 후 입력
              </div>
              {viewField && (
                <div style={styles.fieldContainer}>{fieldElement}</div>
              )}
            </div>
          </div>
          <div style={{ display: "flex", gap: "0.7rem" }}>
